...
namespace ProcurementHTE.Infrastructure.Repositories
{
    public class DashboardRepository : IDashboardRepository
    {
        ...
        )
        {
            ...
            {
                // Filter based on role and assigned user
                query = query.Where(p =>
                    (isAnalyst && p.ProcurementStatus == ProcurementStatus.WaitingApprovalAnalyst && p.AnalystHteUserId == userId) ||
                    (isAsstManager && p.ProcurementStatus == ProcurementStatus.WaitingApprovalAsstManager && p.AssistantManagerUserId == userId) ||
                    (isManager && p.ProcurementStatus == ProcurementStatus.WaitingApprovalManager && p.ManagerUserId == userId)
                );
            }
