...
namespace ProcurementHTE.Infrastructure.Repositories {
  public class WorkOrderRepository : IWorkOrderRepository {
    ...
    public async Task<List<Tender>> GetTendersAsync() {
      return await _context.Tenders.OrderBy(t => t.TenderName).ToListAsync();
    }
    ...
    public async Task UpdateWorkOrderAsync(WorkOrder wo) {
      ...
      try {
        ...
        var exists = await _context.WorkOrders.AnyAsync(w => w.WorkOrderId == wo.WorkOrderId);
        if (!exists) {
          throw new KeyNotFoundException($"Work Order dengan ID {wo.WorkOrderId} tidak ditemukan");
        }
        throw new InvalidOperationException("Data telah diubah oleh user lain. Silakan refresh dan coba lagi", ex);
      }
    }
