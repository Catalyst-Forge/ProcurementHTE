...
namespace ProcurementHTE.Core.Services {
    public class FileSystemTemplateProvider : ITemplateProvider {
        ...
        public FileSystemTemplateProvider(ILogger<FileSystemTemplateProvider> logger) {
            ...
            _templatesPath = Path.Combine(Directory.GetCurrentDirectory(), "Templates", "Documents");
        ...
        public async Task<string> GetTemplateAsync(string templateName, CancellationToken ct = default) {
            ...
            _logger.LogDebug("Loading template: {TemplateName} from {Path}", templateName, filePath);

            if (!File.Exists(filePath)) {
                _logger.LogError("Template not found: {Path}", filePath);
                throw new FileNotFoundException($"Template '{templateName}' tidak ditemukan di {filePath}");
            }

            try {
                var content = await File.ReadAllTextAsync(filePath, ct);
                _logger.LogDebug("Template loaded successfully: {TemplateName}, Size: {Size} chars", templateName, content.Length);
                return content;
            } catch (Exception ex) {
                _logger.LogError(ex, "Error reading template: {TemplateName} from {Path}", templateName, filePath);
                throw new InvalidOperationException($"Gagal membaca template '{templateName}': {ex.Message}", ex);
            }
        }
    }
}
