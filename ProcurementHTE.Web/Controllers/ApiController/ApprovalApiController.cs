...
namespace ProcurementHTE.Web.Controllers.ApiController
{
    ...
    public class ApprovalApiController : ControllerBase
    {
        ...
        public async Task<IActionResult> UpdateStatusByQrCode([FromBody] UpdateByQrRequest req, CancellationToken ct)
        {
            ...
            ApprovalUpdateResult result =
                await _svc.UpdateStatusByQrAsync(req.QrText ?? "", req.Action ?? "", req.Note, user, ct);

            return Ok(result);
        }

        [HttpPost("update-status-by-approval-id")]
        public async Task<IActionResult> UpdateStatusByApprovalId([FromBody] UpdateByApprovalIdRequest req, CancellationToken ct)
        {
            if (req is null) return BadRequest(new { ok = false, message = "Body tidak boleh kosong." });
